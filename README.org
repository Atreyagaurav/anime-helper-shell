...
* NOTE:
  ~play~,, ~download~, ~continue~ commands don't work currently because of changes in the gogoanime website which was the only source till now.

  I have added a ~web~ command to browse the episode page and watch it in the browser for now.
